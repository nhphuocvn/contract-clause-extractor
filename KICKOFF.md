...
- **Cash-layer refinement** (`ce54a76`) — replaced the whole-quarter DSO approximation with a **monthly working-capital cash layer** covering all three legs: collections (DSO), supplier payments (DPO), and inventory build (lead time). Revenue recognition stays quarterly; only cash timing is monthly, with within-quarter activity spread evenly across 3 months. net-30/60/90 now produce genuinely different collection schedules, peak receivables, NPV, and working-capital draw. DPO and inventory lead net into one explicit `cogs_cash_lag = dpo_months − inventory_lead_months` (defaults: net-60 DPO → 2mo, 3mo inventory lead → **−1mo**, COGS cash funded one month ahead of shipment). New library defaults `supplier_payment_dpo_days` (60; owner Treasury/Procurement) and `inventory_lead_months` (3; owner Operations/Supply Chain), each owner/type-tagged for the §5 register. New additive output `ScenarioResult.peak_working_capital_draw_usd`. Operating NPV documented as **pre-tax** (after-tax = roadmap); **perfect collections** assumed (no bad debt/disputes). **Re-pin:** BASE deployment payback Q5→**Q6** under the realistic WC drag (intended — the fix working); financed payback Q0 unchanged. Hand-calc pins: peak receivables net-30/60/90 = **$166.667M / $333.333M / $500.000M**; deployment NPV = **$774.661M / $749.974M / $725.483M** (net-30 vs net-90 = **$49.178M** NPV cost); peak WC draw net-90 = **−$157.0M**; inventory build = **$35.0M/month** funded ahead of first ship.

- **Phase 6 — policy, benchmarks, reports** (`44afbec`) — six modules + three data files + schema additions. `policy_engine.py` + `data/crb_policy.json` (per-rule PASS/ESCALATE/BLOCK + approver routing; pure `evaluate_policy(signals, rules)`, one evaluator per `PolicyRuleKind`, worst-of aggregation, ordered approver union; margin floor tests the **cash** blended margin so the warrant is not double-counted). `benchmarks.py` + `data/benchmarks.json` (pure compare + staleness >2 quarters; missing-file → labeled absence). `assumption_register.py` (§5 register: every input typed + owner-tagged; owner/`assumption_type` promoted to first-class `AssumptionProvenance` fields, set from the library). `assumption_gap_report.py` (ranked clarifying questions reusing the tornado + warrant range; policy numbers excluded). `crb_memo.py` (pure `build_crb_memo` payload + deterministic `render_crb_memo_markdown` fallback; warrant section carries the §4 correlation caveat). `glossary.py` + `data/glossary.json` (38 terms). New schemas: `AssumptionType`, `RegisterEntry`, `AssumptionGapLine`, `Benchmark`, `BenchmarkComparison`; `CRBMemo` extended (warrant_section, policy_verdict, gap_report_lines). **Live verification (synthetic deal):** verdict **ESCALATE**, approvers **CFO + General Counsel + Treasury**; top gaps **COGS $225.0M (cost accounting)** then **rebate $41.0M (Legal)**; benchmarks show margin 37.6% < 42% median and the stale take-or-pay benchmark flagged. Hand-calc pins: blended margin **37.63%**, committed value **$3.75B**, rebate ambiguity **$41.0M**, COGS tornado **±$225.0M**, warrant valuation range width **$1.974B**.

- **Phase 7 — Excel export** (`1e4154a`, latest) — `deal_copilot/excel_export.py` (`build_workbook()`) generates an 11-tab openpyxl workbook where every computed cell is a live formula referencing named-range assumption cells (§9.1 circuit principle). 54 named ranges (ASP, UnitQ1–Q12, TierTable, RebateToggle, ContraSchedQ1–Q12, VestProbT1–T4, …) wire assumption changes through the whole model. Reading A: prospective marginal-tier rebate via MAX/MIN zone arithmetic. Reading B: retroactive via VLOOKUP on year-end cumulative (not quarter-end), matching the engine exactly. A/B toggle: single IF per quarter, no nested IFs. Warrant contra-revenue: live formula `=-TotalEFV*UnitQn/SUM(UnitQ1:UnitQ12)`; stock-price edit on Warrant_Assump tab propagates to Model!Net_Revenue automatically. Scenarios: BASE fully live (`=Model!$N$22` etc.); DOWNSIDE/UPSIDE/ET are static engine snapshots marked with a loud amber banner. Cell style convention: white = live formula, `#FFF2CC` amber = static snapshot, `#FF9900` bright amber = staleness banner; legend on Assumptions tab. Sheet protection: all sheets locked; input cells explicitly unlocked. `tests/test_excel_export.py`: 41 golden-file tests (tab names, 9 named-range addresses, 8 formula-presence checks, exact `=B3*B4` chain, Reading B year-end VLOOKUP column for Q1/Q5/Q9, warrant contra denominator sanity, BASE live/DOWNSIDE static, $1,357.5M GM anchor, all 12 quarterly unit seeds).

- **Phase 7 REVIEW — RESOLVED** (`f22f6e6` engine, `383d07f` Excel) — the tab-by-tab review's findings were fixed in two commits. **Engine (`f22f6e6`, alone):** per-scenario warrant contra-revenue. The contra schedule is recomputed on each scenario's actual deployed units (the warrant vests on deployment, Warrant §2) instead of reusing the BASE full-deployment schedule on every scenario. DOWNSIDE contra drops from the full **$3,383.93M** to **$2,843.44M** (UPSIDE stays full — 172.5k units > 150k final milestone); the −$2.15B / 463% garbage is gone. Cash-view GM% is sane (~37–42%); **GAAP gross margin stays negative across all scenarios — the correct headline** (the $3.38B equity grant exceeds the $1.36B product margin; the deal is cash-positive but GAAP-dilutive). 16 hand-calc-pinned tests (`tests/test_scenario_warrant_margins.py`). **Excel rebuild (`383d07f`):** `excel_export.py` rebuilt as a 14-tab fully-live, self-documenting "LC4 chip". Liveness mandate — zero frozen numbers in the calc path: every input is an editable named-range pin and every downstream number is a formula reading it; **NPV is a live `=NPV()`** driven by WACC; the **warrant contra is a live deployment-band allocation** that scales with a new **Demand% dial** (verified: sheet contra = $3.3839B at 100%, $2.8434B at 75%, matching the engine — the mis-scaling bug is structurally impossible in the sheet); the warrant EV range is live on price + vest probs. The only static cells are labeled input pins and the Scenarios DOWNSIDE/UPSIDE/ET snapshots (banner-labeled "change Demand% to explore live"). Two documentation layers: visible ELI5 + verbatim clause text (real §-text via new `deal_copilot/demo_deal.py`) + example + owner + provenance columns; openpyxl hover comments on calc cells. New tabs/fixes: Cover header stamp (USD actual dollars, NPV pre-tax, confidential, reading guide, legend); rebate reading default **B-Retroactive** with both readings + IF selector; probability %-formats + live sum-check flag; single `TotalUnits` cell; live monthly AR roll-forward; live take-or-pay floor block; fully-populated CRB_Summary; loud cash↔GAAP bridge; two narrative analysis tabs (finance-manager + plain-English). New `scripts/build_demo_workbook.py`. Excel golden tests rewritten content/named-range-based with per-input liveness "wiggle" tests.

- **Phase 7 polish — workbook cleanliness & editability** (this session) — two demo-blocking fixes on `excel_export.py`, no engine/test changes. **(1) `#NAME?` cells eliminated:** several plain-English notes (the Model "Note / source" column, rebate-headroom and prepayment helper notes) were written into cells starting with `=`, so Excel parsed the sentence as a formula and showed `#NAME?` (e.g. `"= start + units shipped this quarter."` → `=@start +@ units…`). Removed the leading `=` from the offending notes and hardened `_wrapped()` (the prose writer, which never carries a formula) to strip any leading `=` defensively. **(2) Sheet protection removed:** dropped all `ws.protection.sheet = True` / `password` blocks from every tab — the workbook now opens fully editable with no password prompt (default to zero protection; the per-cell `Protection(locked=False)` hints are now harmless no-ops). New `scripts/_verify_no_error_cells.py` proves it: a cell-by-cell scan of the rebuilt `deal_model_demo.xlsx` reports **0 cells starting with `#`** and **0 prose-as-formula cells** across all 14 tabs, and every tab's `protection.sheet` is `False`.

**All financial math is hand-calc pinned: `204 tests passing` (139 prior + 16 engine scenario-margin + 49 Excel).**
...
openpyxl workbook:
- **Assumptions tab:** every input, provenance class color-coded, mandatory Source/Basis column. Named ranges for key inputs. (Sheet protection was intentionally dropped — see Phase 7 polish — so the workbook opens fully editable; the named-range/circuit design, not cell-locking, is what keeps it traceable.)
